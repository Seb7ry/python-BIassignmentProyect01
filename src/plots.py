...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    # Graficar la cantidad de pedidos por día usando 'order_count' en lugar de 'order_amount'
    plt.plot(df['date'], df['order_count'], label='Order Count per Day', color='blue')
    ...
    plt.title('Order Count per Day with Holidays Marked')
    plt.xlabel('Date')
